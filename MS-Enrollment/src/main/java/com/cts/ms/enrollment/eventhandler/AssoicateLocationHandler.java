...
public class AssoicateLocationHandler {
	...
	public void listenToLocationCreatedEvent(LocationCreatedEvent createdEvent) {
		try {
			...
			LoggingHelpUtil
					.printDebug("The locationId returned is --->" + createdEvent.getLocationBO().getLocationId());

			enrollmentBL.associateEnrollmentWithLocation(createdEvent.getEnrollmentId(),
					createdEvent.getLocationBO().getLocationId().toString());
		} catch (Exception ex) {
